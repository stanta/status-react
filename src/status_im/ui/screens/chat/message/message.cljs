...
(defmethod ->message constants/content-type-text
  [{:keys [content outgoing current-public-key public?] :as message} {:keys [on-long-press close-modal modal]}]
...
(defmethod ->message constants/content-type-emoji
  [{:keys [content current-public-key outgoing public?] :as message} {:keys [on-long-press close-modal modal]}]
  (let [response-to (:response-to content)]
    [message-content-wrapper message
     [react/touchable-highlight {:on-press      (fn [_]
                                                  (re-frame/dispatch [:chat.ui/set-chat-ui-props {:input-bottom-sheet nil}])
...
(defmethod ->message constants/content-type-sticker [{:keys [content] :as message} {:keys [on-long-press]}]
     ...
     [react/touchable-highlight {:on-press      (fn [_]
                                 ...
                                 :on-long-press (fn []
                                                  (on-long-press
                                                   [{:on-press #(when pack
                                                                  (re-frame/dispatch [:stickers/open-sticker-pack pack]))
                                                     :lable    (i18n/label :t/view-details)}]))}
      [react/image {:style  {:margin 10 :width 140 :height 140}
                   ;;TODO (perf) move to event
                    :source {:uri (contenthash/url (-> content :sticker :hash))}}]]]))

(defmethod ->message constants/content-type-image [{:keys [content] :as message} {:keys [on-long-press]}]
  [message-content-wrapper message
   [react/touchable-highlight {:on-press      (fn [_]
                                                (when (:image content)
