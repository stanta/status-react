...
(defn picker [{:keys [outgoing on-copy on-reply send-emoji]}]
  [animated/view {:style (container-style {:outgoing outgoing})}
   [rn/view {:style (reactions-picker-row)}
...
(defn reaction [{:keys [outgoing]} {:keys [own emoji-id quantity]}]
  [rn/view {:style (reaction-style {:outgoing outgoing
                                    :own      own})}
   [rn/image {:source (get reactions emoji-id)
              :style  {:width        16
                       :height       16
                       :margin-right 4}}]
   [quo/text {:style (reaction-quantity-style {:own own})}
    quantity]])

(defn message-reactions [message reactions]
  (when (pos? (count reactions))
    [rn/view {:style (reactions-row message)}
